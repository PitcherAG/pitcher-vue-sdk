...
export const Template = (args, { argTypes }) => {
    return {
        ...
        components: { OldCalendar },
        template: '<OldCalendar v-bind="$props" />'
    }
} 

<!-- Define story settings here -->
<Meta 
    title="Components/OldCalendar"
    component={OldCalendar}
    argTypes={argTypes}
    args={args}
/>
...
### Usage
```javascript
import { OldCalendar } from '@pitcher/vue-sdk'
...
```html
<!-- Simple usage -->
<OldCalendar v-model="selectedDate" />

<!-- Using fluid, fixed input width, smaller input size -->
<OldCalendar v-model="selectedDate" fluid size="small" max-width="300px" />
 
<!-- Using with event dates -->
<OldCalendar v-model="selectedDate" :event-dates="eventDates" />

<!-- Using with disabled dates -->
<OldCalendar v-model="selectedDate" :disabled-dates="disabledDates" />
```
